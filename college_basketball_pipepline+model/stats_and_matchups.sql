...
WITH unioned AS (
    SELECT 
        ...
        IF(neutralsite = FALSE, 1, 0) AS home_game,
...
-- For each team, get the latest ingested_at on or before each game date
, game_rating_dates AS (
    ...
    FROM deduped g
    CROSS JOIN (SELECT DISTINCT teamid, ingested_at FROM `project-bf3ebdef-3dea-4c35-a21.mbb.ratings`) r
...
, game_stats_dates AS (
    ...
    FROM deduped g
    CROSS JOIN (SELECT DISTINCT teamid, ingested_at FROM `project-bf3ebdef-3dea-4c35-a21.mbb.team_stats`) r
    GROUP BY g.id, g.home_team_id, g.away_team_id, g.start_date
)
...
, home_rating AS (
    ...
    LEFT JOIN `project-bf3ebdef-3dea-4c35-a21.mbb.ratings` r
...
, home_stats AS (
    ...
    LEFT JOIN `project-bf3ebdef-3dea-4c35-a21.mbb.team_stats` r
        ON r.teamid = g.home_team_id
...
, lines_dedupe AS (
    ...
    FROM `project-bf3ebdef-3dea-4c35-a21.mbb.lines`
)

-- Step 1: unpivot games so each team has one row per game
, team_games AS (
    SELECT
        id AS game_id,
        start_date,
...
-- Step 2: calculate rolling stats per team
, team_rolling AS (
    SELECT
        game_id,
        team_id,
...
-- Step 3: join back to your main query twice
, home_form AS (
...
SELECT 
    ...
    home_effectiveFieldGoalPct - away_effectiveFieldGoalPct fg_perc_diff,
    home_freeThrowRate - away_freeThrowRate ft_rate_diff,
    home_offensiveReboundPct - away_offensiveReboundPct orb_perc_diff,
    home_turnoverRatio - away_turnoverRatio to_ratio_diff,
FROM away_stats r
